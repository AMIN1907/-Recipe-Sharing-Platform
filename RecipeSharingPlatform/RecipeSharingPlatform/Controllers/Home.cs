...
namespace RecipeSharing.Controllers
{
    ...
    public class Home : ControllerBase
    {
        ...
        public Home(iRecipeRepository RecipeRepository, ImangeprofileRepository ProfileRepository)
        {
...

        {
            ...
            var Recipelist = new HashSet<RecipeResponse>();
            var eventList = await _RecipeRepository.GetAllTEntity<evnet>();
            var eventResults = eventList.GroupBy(e => e.Rid)
                                         .OrderByDescending(g => g.Count())
                                         .Take(10)
                                         .SelectMany(g => g)
                                         .ToHashSet();
            var Recipesit =new HashSet<Recipe>();
            foreach (var Result in eventResults)
            {
                var existingRecipe = await _RecipeRepository.GetSpecialEntity<Recipe>(e => e.RecipeId == Result.Rid);

                Recipesit.Add(existingRecipe);
            }


            var userResults = await _ProfileRepository.GetUserFolloweing(await _ProfileRepository.GetSpecialEntity<ApplicationUser>(e => e.Id == userId));
            foreach (var Result in userResults)
            {
                var existingRecipe = await _RecipeRepository.GetSpecialEntity<Recipe>(e => e.UserId == Result);
            ...
            foreach (var recipe in Recipesit)
            {
